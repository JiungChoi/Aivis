...
export default function HomePage() {
  ...
  async function handleAddSuggestionToSchedule(title: string) {
    const today = new Date().toISOString().split('T')[0];
    const now   = new Date();
    // 다음 정시로 설정
    const nextH = new Date(now);
    nextH.setHours(nextH.getHours() + 1, 0, 0, 0);
    const startTime = `${String(nextH.getHours()).padStart(2, '0')}:00`;
    await scheduleService.create({ date: today, startTime, title, category: 'Work' });
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{
      ...
      <div className="flex-1 flex overflow-hidden" style={{ userSelect: dragging.current ? 'none' : 'auto' }}>
        ...
        {/* ── COL 2: 시간 분석 + 작업 제안 ── */}
        <div className="overflow-y-auto p-4 space-y-4" style={{ width: col2, flexShrink: 0, overflow: 'hidden', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
          <TimeAnalysisPanel
            schedule={schedule}
        ...
        <div className="flex-col overflow-hidden" style={{ flex: '1 1 0', minWidth: 160, display: 'flex', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
          <div className="flex items-center gap-0 flex-shrink-0 px-4 pt-4 pb-0">
            <span style={{
              color: 'rgba(235,235,245,0.35)',
              fontWeight: 600,
              fontSize: 11,
            ...
            <div className="flex gap-0.5 overflow-x-auto">
              {NEWS_TABS.map((tab) => (
                <button key={tab} onClick={() => setNewsTab(tab)}
                  className={`flex-shrink-0 text-[10px] px-2.5 py-1.5 rounded-t-lg transition-colors border-b-2 ${
                    newsTab === tab
                      ? 'text-blue-400 border-blue-400 bg-blue-500/10'
                      : 'text-gray-600 border-transparent hover:text-gray-400 hover:bg-white/5'
                  }`}>
                  {tab}
                </button>
              ))}
          ...
          <div className="mx-4" style={{ borderTop: '1px solid rgba(84,84,88,0.25)' }} />
